...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:csv="csv:csv" xmlns:tei="http://www.tei-c.org/ns/1.0" version="3.0">
 ...
 <xsl:variable name="works" select="//tei:listBibl"/>
        ...
        <xsl:variable name="sourcedocuri" select="concat('xmldb:exist://admin:jeanluc23@45.56.98.26:8080/exist/xmlrpc/db/madrid/xml/',$workrefshortlc,'people.xml')"/>
        <xsl:variable name="sourcedoc" select="doc($sourcedocuri)"/>
        <xsl:variable name="sourcedocperson" select="$sourcedoc//tei:person"/>
            <xsl:result-document href="xmldb:exist://admin:jeanluc23@45.56.98.26:8080/exist/xmlrpc/db/madrid/csv/people-{$workreflc}.csv">
